...
SELECT  OWNER                                                                                                                                       AS ADDRESS
       ...
       ,ENS_NAME
       ,REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(GITHUB,'https',''),'http',''),'github.com',''),':',''),'/',''),'@',''),chr(0),'')   AS GITHUB_USERNAME
       ,REPLACE(REDDIT,chr(0),'')                                                                                                                   AS REDDIT_USERNAME
       ,REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(TELEGRAM,'https',''),'http',''),'t.me',''),':',''),'/',''),'@',''),'#',''),chr(0),'') AS TELEGRAM_USERNAME
       ,REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(TWITTER,'https',''),'http',''),'twitter.com',''),':',''),'/',''),'@',''),chr(0),'') AS TWITTER_USERNAME
FROM crosschain.core.ez_ens
WHERE ( EMAIL IS NOT NULL OR GITHUB_USERNAME IS NOT NULL OR TWITTER_USERNAME IS NOT NULL OR TELEGRAM_USERNAME IS NOT NULL )
AND ADDRESS IN ({0})
"""
